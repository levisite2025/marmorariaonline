...
import { AlertCircle, ArrowRight, Box, Eye, EyeOff, Lock, User } from 'lucide-react';

interface LoginScreenProps {
  onLogin: (session: { username: string; role: 'admin' | 'operator' }) => void;
}
...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin }) => {
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    window.setTimeout(() => {
      ...
      if (normalizedUser === 'operador' && password === '123456') {
        onLogin({ username: 'operador', role: 'operator' });
        return;
      }
        ...
        <div className="p-8 pt-10">
          <div className="flex flex-col items-center mb-8">
            ...
            <h1 className="font-bold text-2xl tracking-tight text-slate-800">
              Marmoraria <span className="text-blue-600">Online</span>
            </h1>
            <p className="text-slate-500 text-sm mt-1 font-medium tracking-wide">Acesso ao estudio comercial</p>
          </div>
          ...
          <div className="mt-8 text-center">
            <p className="text-xs text-slate-400">Ambiente de demonstracao</p>
            <p className="text-[10px] text-slate-300 mt-1">(Admin: admin / 123456 | Operador: operador / 123456)</p>
          </div>
        </div>
      </div>
    </div>
